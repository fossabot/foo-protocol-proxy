...
clean: clean-bin clean-coverage clean-files clean-version clean-tests ## to clean up all generated files.

format: ## to format all Go files.
	@echo "$(INFO_COLOR)$(MSG_PREFIX) Formatting code$(MSG_SUFFIX)$(NO_COLOR)"
	@test -z "$$($(GO_FMT) -s -w $(GO_FLAGS) $(GO_FILES) 2>&1 | tee /dev/stderr)"

help: ## to get help about the targets.
	@$(call PROJECT_LOGO,$(OK_COLOR)) 2>&1
	@echo "$(INFO_COLOR)Please use \`make <target>\`, Available options for <target> are:$(NO_COLOR)\n"
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z._-]+:.*?## / {sub("\\\\n",sprintf("\n%22c"," "), $$2);printf "  $(STAR) $(HELP_COLOR)%-28s$(NO_COLOR) %s\n", $$1, $$2}' $(MAKEFILE_LIST) | sort -u 2>&1
	@echo "\nUseful variables:\n"
	@awk 'BEGIN { FS = "[:?]=" } /^## /{x = substr($$0, 4); getline; if (NF >= 2) printf "  $(PLUS) $(DISCLAIMER_COLOR)%-29s$(NO_COLOR) %s\n", $$1, x}' $(MAKEFILE_LIST) | sort -u 2>&1

list: ## to list all targets.
	@awk -F':' '/^[a-z0-9][^$#\/\t=]*:([^=]|$$)/ {split($$1,A,/ /);for(i in A)printf "$(LIST_COLOR)%-30s$(NO_COLOR)\n", A[i]}' $(MAKEFILE_LIST) | sort -u 2>&1

nuke: clean ## to do clean up and enforce removing the corresponding installed archive or binary.
	@echo "$(INFO_COLOR)$(MSG_PREFIX) Cleaning up Go dependencies$(MSG_SUFFIX)$(NO_COLOR)"
